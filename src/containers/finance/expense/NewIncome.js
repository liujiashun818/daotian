...
import NewIncomeType from './NewIncomeType';
...
class NewIncomeModal extends BaseModal {
  constructor(props) {
    super(props);
    this.state = {
      visible: this.props.incomeShow == '1',
      expenseTypes: [],
      formData: {},
      user: [],
    };
  ...
  saveNewType(newItem) {
    api.ajax({
      ...
      type: 'POST',
      data: { name: newItem, type: 0 },
    }, data => {
      this.getIncomeTypes(data.res.sub_type._id);
    });
  }

  getIncomeTypes(newTypeId) {
    api.ajax({ url: api.finance.getProjectTypeList(0) }, data => {
  ...
  render() {
    ...
    return (
      <span>
        ...
        <Modal
          title={<span><Icon type="plus" className="mr10" />新增收入</span>}
          visible={visible}
          width="720px"
        ...
        >
          <Form>
            <Row>
              <Col span={10}>
                <FormItem label="收款日期" {...formItemThree}>
              ...
              <Col span={9}>
                <FormItem label="收入项目"  {...formItemThree}>
              ...
              <Col span={4} offset={1}>
                <FormItem label="">
                  <p className="ant-form-text">
                    <NewIncomeType save={this.saveNewType.bind(this)} />
                  </p>
                </FormItem>
              </Col>
            </Row>
            ...
            <Row>
              <Col span={10}>
                <FormItem label="收款金额" {...formItemTwo}>
            ...
            <Row>
              <Col span={10}>
                <FormItem label="付款方" {...formItemTwo}>
                {getFieldDecorator('payer', {
                  rules: FormValidator.getRuleNotNull(),
                  validateTrigger: 'onBlur',
                })(
                  <Input placeholder="付款方" style={{ width: 165 }} />,
                )}
                </FormItem>
              </Col>
...
NewIncomeModal = Form.create()(NewIncomeModal);
